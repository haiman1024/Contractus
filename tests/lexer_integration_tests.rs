...
fn test_array_syntax() {
    ...
    assert_eq!(tokens[11].kind, TokenKind::Comma);
    assert_eq!(tokens[12].kind, TokenKind::IntLiteral(2));
}

#[test]
fn test_error_handling() {
    let input = "let x = @;"; // @ 是无效字符
    let lexer = Lexer::new(input);
...
fn test_unterminated_string() {
    let input = r#"let s = "unterminated string"#;
    let lexer = Lexer::new(input);
    let result = lexer.tokenize();
...
fn test_position_tracking() {
    ...
    // 验证位置信息
    assert_eq!(tokens[0].span.line, 1); // fn 在第1行
    assert_eq!(tokens[1].span.line, 2); // main 在第2行
    assert_eq!(tokens[2].span.line, 3); // ( 在第3行
    assert_eq!(tokens[3].span.line, 3); // ) 在第3行
}
